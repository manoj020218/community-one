...
/** Self-contained MCR module-enablement check — mirrors the platform's module-registry gating pattern without depending on another module's shared hook. */
export function useMcrModule() {
  ...
  const module = query.data?.find((item) => item.code === 'MCR');
