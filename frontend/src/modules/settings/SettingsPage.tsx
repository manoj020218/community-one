...
import { Settings, Bell, Globe, Shield, Save, Activity, MessageSquare, Users, UserCheck } from 'lucide-react';
...
import { MembersManagementPanel } from '../users/MembersManagementPanel';
import { VisitorSettingsPanel } from '../visitor/VisitorSettingsPanel';
import { CommunicationMediaTab } from './CommunicationMediaTab';

const TIMEZONES = ['Asia/Kolkata', 'Asia/Dubai', 'UTC', 'America/New_York', 'Europe/London'];
const CURRENCIES = ['INR', 'USD', 'AED', 'GBP', 'EUR'];

type TabKey = 'general' | 'health' | 'communication' | 'members' | 'visitor';
...
export function SettingsPage() {
  ...
  const tabs: Array<{ key: TabKey; label: string; icon: typeof Settings }> = [
    ...
    ...(hasPermission(user, 'communication.settings.manage') ? [{ key: 'communication' as const, label: 'Communication Media', icon: MessageSquare }] : []),
    ...(hasPermission(user, 'user.create') || hasPermission(user, 'user.read') ? [{ key: 'members' as const, label: 'Members Mgmt', icon: Users }] : []),
    ...(hasPermission(user, 'visitor.settings.manage') ? [{ key: 'visitor' as const, label: 'Visitor (VMS)', icon: UserCheck }] : []),
  ];
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Settings" subtitle="Configure platform, society, and communication preferences" />
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-100 pb-3">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-medium transition-colors ${activeTab === tab.key ? 'bg-primary-50 text-primary-700' : 'text-slate-500 hover:bg-slate-50'}`}
          >
      ...
      {activeTab === 'health' && <div className="space-y-6">{<HealthTab />}</div>}
      {activeTab === 'communication' && <CommunicationMediaTab />}
      {activeTab === 'members' && <MembersManagementPanel />}
      {activeTab === 'visitor' && societyId && (
        <div className="card p-6 max-w-2xl">
          <h3 className="font-semibold text-slate-700 mb-5">Visitor Management Settings</h3>
          <VisitorSettingsPanel societyId={societyId} />
        </div>
      )}
    </div>
  );
